...
bool processInput(bool continueApplication) {
	...
	if (modelSelected == 3 && glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS) {
		numAnimacion = 1;
		//rotKizuna = 180.0f;
		modelMatrixKizuna = glm::translate(modelMatrixKizuna, glm::vec3(0.0, 0.0, 0.015));
	}
	else if (modelSelected == 3 && glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS)
	{
		...
		modelMatrixKizuna = glm::rotate(modelMatrixKizuna, glm::radians(1.0f), glm::vec3(0, 1, 0));
	}
...
void applicationLoop() {
	...
	modelMatrixKizuna = glm::translate(modelMatrixKizuna, glm::vec3(5.0, 0.0, 2.0));
		...
		for (colIt = collisionDetection.begin(); colIt != collisionDetection.end();
			...
			if (jt != collidersOBB.end()) {
				if (!colIt->second)
					...
					if (jt->first.compare("kizuna") == 0)
						modelMatrixKizuna = std::get<1>(jt->second);
