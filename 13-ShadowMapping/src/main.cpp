...
//Shader para dibujar la escena su buffer de profundidad
Shader shaderDepth;
...
void applicationLoop() {
	...
	while (psi) {
		...
		/*******************************************
		* Paso 1: Se dibuja la escena desde la luz
		********************************************/
		...
		/*******************************************
		* Debug: Visualizar el mapa de sombras
		********************************************/
		// reset Viewport
		/*glViewport(0, 0, screenWidth, screenHeight);
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
		// Render el buffer de profundidad desde la luz en una primitiva cuadrada
		...
		********************************************/
		...
		glBindTexture(GL_TEXTURE_2D, depthMap);
		shaderMulLighting.setInt("shaaderMap", 10);
...
void renderScene(bool renderParticles){
	...
	for (std::map<float, std::pair<std::string, glm::vec3> >::reverse_iterator it = blendingSorted.rbegin(); it != blendingSorted.rend(); it++) {
		...
		else if (renderParticles && it->second.first.compare("heli") == 0) {
			// Helicopter
